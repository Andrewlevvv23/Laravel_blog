...
@section('content')
    ...
    <main class="blog-post">
        <div class="container">
            ...
            <p class="edica-blog-post-meta" data-aos="fade-up" data-aos-delay="200">{{ $date->translatedFormat('F') }} • {{ $date->day }} • {{ $date->year }} • {{ $date->format('H:i') }} • {{ $post->comments->count() }} - коментаріїв </p>
                ...
                <div class="col-lg-9 mx-auto">
                    <section class="related-posts">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4" data-aos="fade-right" data-aos-delay="100">
                                ...
                                <a class="row" href="{{ route('post.show', $relatedPost->id) }}"> <h5 class="post-title"> {{ $relatedPost->title }} </h5></a>
                            </div>
                            @endforeach
                        </div>
                    </section>
                    ...
                    <section class="py-3">
                        <div  class="d-flex justify-content-center">
                            <p class="blog-post-category fs-5"> Чи подобається вам даний пост? - </p>
                    ...
                    <section class="comment-list mb-5">
                        <h2 class="section-title mb-5" data-aos="fade-up"> Коментарі користувачів: {{ $post->comments->count() }} </h2>
                        @foreach($post->comments as $comment)
                            <div class="comment-text mb-4">
                                <span class="username">
                                    <div> {{ $comment->user->name }} </div>
                                  <span class="text-muted float-right"> {{\Carbon\Carbon::parse($comment['created_at'])->diffForHumans() }} </span>
                                </span><!-- /.username -->
                                    {{ $comment->message }}
                            </div>
                        @endforeach
                        ...
                        <form action="{{ route('comment.store', $post->id) }}" method="post"> @csrf
                            <div class="row">
                                <div class="form-group col-12" data-aos="fade-up">
                                    ...
                                    <textarea name="message" id="message" class="form-control" placeholder="Введіть свій коментар" rows="8"> </textarea>
                                </div>
                            </div>

                            <div class="row">
                                <div class="col-12" data-aos="fade-up">
                                    <input type="submit" value="Опублікувати" class="btn btn-warning">
                                </div>
                            </div>
                        </form>
                    </section>
